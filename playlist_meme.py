...
def find_song_with_name(phrase):
	"""
	find_song_with_name(phrase String) -> spotify_song_object
	This function looks up if a song exists with the following phrase. 
	This implementation uses case insensitive direct match, this means 
	that if the keywoard "hey," will count as one word and match exactly.
	The funciton will always return the first match.
	"""
	if len(phrase) == 0:
		return None 
